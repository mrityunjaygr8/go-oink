...
func main() {
	...
	err = repo.UserRepository.UserUpdatePassword(context.Background(), u.ID, "drowssap")
	logger.Info().Err(err).Msg("Update Password")
	err = repo.UserRepository.UserAuthenticate(context.Background(), user.Email, "password")
	logger.Info().Err(err).Msg("Authenticate")
