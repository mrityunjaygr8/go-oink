...
func testTokenToOneUserUsingTokenUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, tokenDBTypes, false, tokenColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Token struct: %s", err)
	}
	...
	local.User = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.TokenUser().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadTokenUser(ctx, tx, false, (*[]*Token)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.TokenUser == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.TokenUser = nil
	if err = local.L.LoadTokenUser(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.TokenUser == nil {
		t.Error("struct should have been eager loaded")
...
func testTokenToOneSetOpUserUsingTokenUser(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		err = a.SetTokenUser(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.User != x.ID {
			t.Error("foreign key was wrong value", a.User)
		}

		zero := reflect.Zero(reflect.TypeOf(a.User))
		reflect.Indirect(reflect.ValueOf(&a.User)).Set(zero)
		...
		if a.User != x.ID {
			t.Error("foreign key was wrong value", a.User, x.ID)
		}
	}
}
...
var (
	tokenDBTypes = map[string]string{`Token`: `uuid`, `User`: `uuid`, `Type`: `character varying`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_            = bytes.MinRead
)
